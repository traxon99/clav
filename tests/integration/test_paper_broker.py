"""PaperBroker tests against a mocked alpaca-py TradingClient.

No live Alpaca paper account is available in this sandbox to record authentic
VCR cassettes (alpaca-py talks HTTP via `requests`, not httpx, so `respx`
couldn't intercept it either way). Instead we mock TradingClient itself and
return alpaca-py's own response model instances — this exercises the adapter's
request-building/mapping/error-handling exactly as a cassette would, without
depending on network access or credentials. See docs/11-testing.md §1.
"""
...
def _broker() -> tuple[PaperBroker, MagicMock]:
    mock_client = MagicMock(spec=TradingClient)
    broker = PaperBroker("key", "secret", client=mock_client)
