...
httpx), and there's no live paper account/API key available in this sandbox to
record authentic VCR cassettes against. Tests instead inject a mocked
``TradingClient`` built from alpaca-py's own response model classes, so the
mapping/error-handling logic here is exercised against exactly-shaped
responses without hitting the network — satisfying "no live network in CI"
the same way a cassette would.
"""
