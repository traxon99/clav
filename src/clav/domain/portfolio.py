...
class PortfolioManager:
    ...
    def apply_fill(
        ...
        *,
        ...
        ``stop_price``/``take_profit_price`` (Story 2.3) are the sizer's
        entry-time levels; they're only applied when a fill **opens** a fresh
        position — ignored for sells and for adding to an already-open one,
        which keeps whatever stop/TP the original entry set.
        """
        ...
        if order_row.side == "buy":
            self._apply_buy(
                instrument, order_row, existing_row, fill, now, stop_price, take_profit_price
            )
        else:
